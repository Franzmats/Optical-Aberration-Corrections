...
def TPEF(E_in, scat, sample): #two photon fluorescence signal changed to one photon changing **4 to **2
    "calculation of two photon signal based on pupil field"
    I2ph = int(eta*1e3*sum(np.abs(sample * (np.fft.fft2(scat * E_in)/N/N))**2))
    return np.random.poisson(I2ph)


def gaussian(N,sigma):  # Create gaussian profile
    x = np.arange(N)
    y = np.arange(N)
    X, Y = np.meshgrid(x, y)    
    center_x = N // 2  # X coordinate of the center
    center_y = N // 2  # Y coordinate of the center
...
for idx in range(trials):
    ...
    if sampletype == 'layer':
        ...
        sample = gaussian(N,0.0005) #sample with gaussian profile
...
title('Phase of the scattering field (original)')

subplot(2, 2, 2)
imshow(angle(scat*C), cmap='hsv',origin='lower')
colorbar(label='Phase (radians)')
title('Phase of corrected wavefront')


phase_diff=angle(scat*C)-angle(scat)
