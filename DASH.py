...
NN = 7#side length of the SLM in pixels; the number of correctable modes is thus N x N
N =14#number of scattered modes = N^2
...
gridtype = 'pupils'  #grid can be square, polar0 (not evenly spaced polar), polar1 (evenly spaced polar), or pupils (as the SLM)
...
if gridtype=='square':
    ...
    for n in range(NN*NN):
        ...
        M[:, :, mm] = resize((r *(Kx*cos(thet)+Ky*sin(thet))),(N,N))
        mm += 1
...
for idx in range(trials):
    ...
    C = ones((N,N), dtype = "complex") #initializing correction mask
