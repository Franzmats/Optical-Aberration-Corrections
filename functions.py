from Complementary import h5_LV_MF_format_extract_parameters, phase_unwrap, zernikeFittype
from scipy.interpolate import interp2d, griddata, NearestNDInterpolator, RectBivariateSpline,Rbf, SmoothBivariateSpline
...
def pupil_segmentation_init(data):
    ...
    # Phase recovery 
    param['phase'] = dict()
    param['phase']['ref'] = [np.floor(np.shape(data['im'])[0]/2+1)-1, np.floor(np.shape(data['im'])[1]/2+1)-1]
...
def generate_filenamePrefix(param):
    ...
    if param['interpolation']['method']:
        if 'zernike':
            stri = stri + ' ' + 'zernike' + ' ' + 'order_' + str(int(param['interpolation']['FitOrder']))
        elif 'thin_plate_smoothing_spline':
...
def retrieve_phase(data,param,out):
    ...
    if param['debug']:
        ...
        plt.autoscale(True)
        plt.colorbar()
        plt.title('Phase from I = a + b * cos(\u03C0 + \u0394)')
        plt.xlabel(r'')
        plt.ylabel(r'')
...
def phase_unwrap_regular(phase_in, ind_keep):
    ...
    phase_unwrapped = np.unwrap(np.unwrap(phase_in, axis=0), axis=1) #phase unwrapping with python method
    #phase_unwrapped=phase_unwrap(phase_in) #this one uses the original matlab phase unwrapping approach
...
def fit_phase(data, param,out):
    ...
    for i in range(phase_unwrapped.shape[0]):
        ...
        ph= popt[0] + popt[1]*x_mesh+ popt[2]*y_mesh
        phase_poly_int_error[i,:,:] = ph - phase_2_fit
        ...
        pointskeep = np.where(points_to_keep[i, :, :])
        popt, _ = curve_fit(poly11, (x_mesh[ pointskeep].flatten(), y_mesh[ pointskeep].flatten()),phase_2_fit[ pointskeep].flatten())
        phase_poly_int[i,:,:]=popt.reshape(phase_poly_int.shape[1],phase_poly_int.shape[2])
        phase_fit[i,:,:]=popt[0] + popt[1]*x_mesh+ popt[2]*y_mesh
      ...
      # Subtract linear term for single pupil fit
        ...
        for i1 in range(len(idx1_rng)):
            for i2 in range(len(idx2_rng)):
                ...
                popt, _=curve_fit(poly11, (x[seg_rng], y[seg_rng]), phase_poly1[0,seg_rng])
                phase_seg_fit = popt[0] + popt[1]*x+ popt[2]*y
...
def average_phase(param,out):
    ...
    for i in range(phase_fit.shape[0]):
        ...
        if averageMethod:
            if 'mean':
                phase_average[i]=np.mean(phase_fit[i,:,:])
            elif 'median':
                phase_average[i]=np.median(phase_fit[i,:,:])
            elif 'sum':
                phase_average[i]=np.sum(phase_fit[i,:,:])
            elif 'single':
...
def unwrap_all_pupils(data, param, out):
    ...
    # Remove tilt   (needs to be 2D) revised curvefit2 and why added the **2 and leastsq
    def poly11(x, a, b, c):
      return a + b*x[0] + c*x[1]

    popt, _=curve_fit(poly11, (x[seg_rng], y[seg_rng]), phase_average[seg_rng])
    phase_seg_fit = popt[0] + popt[1]*x+ popt[2]*y
    phase_zero_tilt = phase_average - phase_seg_fit

    #remove defocus
    def phase_seg_fit_func(xy, a, b):
        x, y = xy
        return a + b * (x**2 + y**2)
    
    popt, _=curve_fit(phase_seg_fit_func, (x[seg_rng], y[seg_rng]), phase_average[seg_rng])

    phase_seg_fit2 = popt[0] + popt[1]*(x**2+ y**2)

    phase_zero_defocus = phase_zero_tilt - phase_seg_fit2
...
def slm_interpolation(data,param,out):
 ...
 #___________________________________________________________________________________ 
    ...
    match param['interpolation']['method']:
        ...
        case 'thin_plate_smoothing_spline':
            tps=ThinPlateSpline(alpha=param['interpolation']['smoothing'])
            tps.fit(np.column_stack((x_int, y_int)), phase_int)
            grid_points = np.column_stack((np.repeat(x_slm, len(y_slm)), np.tile(y_slm, len(x_slm))))
            phase_slm=tps.transform(grid_points)
            phase_slm = phase_slm.reshape(len(x_slm), len(y_slm))
 ...
 #___________________________________________________________________________________ 
    plt.subplot(2,2,3)
    plt.imshow(phase_slm.T, extent=(x_slm.min(), x_slm.max(), y_slm.min(), y_slm.max()), vmin=phase_int.min(), vmax=phase_int.max(),origin='lower')
    plt.colorbar()
    plt.plot(x_int, y_int, '.w')
    plt.plot(beam_center[1], beam_center[0], 'x', markersize=7, linewidth=2)
 ...
 #___________________________________________________________________________________
    match param['interpolation']['method']:
        ...
        case 'thin_plate_smoothing_spline':
            ...
            grid_points = np.column_stack((np.repeat(x_mesh_surf, len(y_mesh_surf)), np.tile(y_mesh_surf, len(x_mesh_surf))))        
            phase_fit_mesh=tps.transform(grid_points)
            phase_fit_mesh= phase_fit_mesh.reshape(len(x_mesh_surf), len(y_mesh_surf))

            phase_fit_at_data_points = np.zeros(len(phase_int))

            for i in range(len(phase_int)):
   
                transformed_point = tps.transform(np.array([[x_int[i], y_int[i]]]))
                phase_fit_at_data_points[i] = transformed_point
    ...
    phase_fit_mesh[phase_fit_mesh < np.min(phase_int)] = np.nan
    phase_fit_mesh[phase_fit_mesh > np.max(phase_int)] = np.nan
    surf=ax.plot_surface(X_mesh_surf, Y_mesh_surf, phase_fit_mesh,cmap='viridis',vmin=phase_int.min(), vmax=phase_int.max(), alpha=0.50)
    ...
    for i in range(len(phase_int)):
        ax.plot([x_int[i], x_int[i]], [y_int[i], y_int[i]], [phase_fit_at_data_points[i], phase_int[i]])
